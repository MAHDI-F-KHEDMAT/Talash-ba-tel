...
import subprocess
import asyncio
import html  # اضافه شده برای رمزگشایی کدهای وب تلگرام
...
def fetch_url(url):
    try:
        ...
        # مدیریت استخراج پیشرفته از کانال‌های تلگرام (پیمایش صفحات قبلی)
        if "t.me/s/" in url:
            all_telegram_links = []
            current_url = url
            channel_id = url.split("/s/")[-1].split("?")[0].strip("/")
            
            for _ in range(5):  # عدد 5 یعنی 5 صفحه به عقب در تاریخچه کانال برود
                resp = requests.get(current_url, headers=headers, timeout=15)
                if resp.status_code != 200:
                    break
                
                content = html.unescape(resp.text)
                all_telegram_links.extend(extract_vless_links(content))
                
                match = re.search(rf'href="/s/{channel_id}\?before=(\d+)"', content)
                if match:
                    current_url = f"https://t.me/s/{channel_id}?before={match.group(1)}"
                else:
                    break
            return list(set(all_telegram_links))

        # منابع معمولی (گیت‌هاب و وب‌سایت‌ها)
...
# --- تست پینگ ثانویه و حذف پینگ‌های نامتعارف ---
...
def main():
    ...
    def worker_xray(node_data):
        ...
        try:
            ...
            return node_data if is_ok else None
        finally:
            port_queue.put(port)
